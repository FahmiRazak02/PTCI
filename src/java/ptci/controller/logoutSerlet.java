...
public class logoutSerlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet logoutSerlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet logoutSerlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       HttpSession session = request.getSession(false); // Get the existing session if it exists
        ...
        out.println("location='index.jsp';"); // Redirect to index.jsp after logout
        out.println("</script>");
                
         // Check for adminID before redirecting to index.jsp
